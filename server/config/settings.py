"""
Django settings for the CamHealth backend.

Configuration is driven by environment variables (see .env.example) so the same
code runs on a developer laptop (SQLite), an on-site clinic server (PostgreSQL),
and the AWS cloud.
"""
...
if os.getenv("DB_ENGINE", "sqlite") == "postgres":
    DATABASES = {
        "default": {
            ...
            "NAME": os.getenv("DB_NAME", "camhealth"),
            "USER": os.getenv("DB_USER", "camhealth"),
